...
NSB.addProperties(hdrFiles);NSB.addDisableProperty(hdrFiles);hdrFiles_left.onclick=function(){ChangeForm(frmBibles)};if(typeof(hdrFiles.onclick)=='function'){if(typeof(hdrFiles_left)!='undefined')hdrFiles_left.onclick=function(){hdrFiles.onclick(hdrFiles_left.getAttribute('nsbvalue'))};if(typeof(hdrFiles_right)!='undefined')hdrFiles_right.onclick=function(){hdrFiles.onclick(hdrFiles_right.getAttribute('nsbvalue'))}};frmFiles.style.display='none';},false);frmFiles.onsubmit=function(event){window.event.stopPropagation();window.event.preventDefault()};NSB.addProperties(frmFiles);frmFiles.onshow=function(){savethefunction_rvar="";NSB.WaitCursor(True);lstFiles.Top=hdrFiles.Height;lstFiles.Height=frmFiles.Height-hdrFiles.Height;lstFiles.deleteItem("all");lstFiles.addItem("Afrikaans");lstFiles.addItem("Amplified Bible");lstFiles.addItem("King James");lstFiles.addItem("World English");lstFiles.addItem("Xhosa");lstFiles.refresh();NSB.WaitCursor(False);return savethefunction_rvar;}
lstFiles.onclick=function(i){savethefunction_rvar="";if(TypeName(i)=="object"){return savethefunction_rvar;}
NSB.WaitCursor(True);var strBible,strName,fileName,req,strImport,dbSource;var strImportDB,strOutput;strName=lstFiles.getItem(i);strName=CStr(strName);strImport=Replace(strName," ","_");strImportDB=strImport+".db";SaveSetting("importbible",strName);strBible=strImport+".js";dbSource=SqlOpenDatabase(strImportDB);if(dbSource==0){NSB.MsgBox("An error was experienced opening the bible named "+strName);NSB.WaitCursor(False);return savethefunction_rvar;}
fileName="http://www.mbangas.com/bibleshow/bibles/"+strBible;req=ReadFile(fileName);if(req.status==200||req.status==0){strOutput=req.responseText;dbImport=JSON.parse(strOutput);SQLImport(dbImport,dbSource,importComplete);}else{NSB.MsgBox("An error was experienced reading the bible named "+strName);NSB.WaitCursor(False);}
return savethefunction_rvar;}
function importComplete(){savethefunction_rvar="";var strName,dbRecord;strName=GetSetting("importbible");strName=Replace(strName,"_"," ");strName=InSingleQuote(strName);dbRecord=new Object();dbRecord["BibleName"]=strName;InsertRecord("Bibles",dbRecord);NSB.WaitCursor(False);return savethefunction_rvar;}
window.addEventListener('load',function(){frmPromise.style.display='block';NSB.List_jqm_init14('promiseL','',true,'100%',false);promiseL_ref=new IScroll(promiseL_scroller,{mouseWheel:true,scrollbars:true,bounce:true,zoom:false});promiseL.refresh=function(){if(typeof promiseL_ref!='undefined')setTimeout(NSB.refresh,100,promiseL_ref)};NSB.addProperties(promiseL,promiseL_scroller);(function(){var d,i,s=''.split(',');for(i=0;i<s.length;i++){if(trim(s[i])!=''){d=document.getElementById('promiseL_'+i);(function(i){d.onclick=function(){ChangeForm(document.getElementById(s[i]))}})(i)}}})()
NSB.addProperties(promiseT);NSB.addDisableProperty(promiseT);if(typeof(promiseT.onclick)=='function'){if(typeof(promiseT_left)!='undefined')promiseT_left.onclick=function(){promiseT.onclick(promiseT_left.getAttribute('nsbvalue'))};if(typeof(promiseT_right)!='undefined')promiseT_right.onclick=function(){promiseT.onclick(promiseT_right.getAttribute('nsbvalue'))}};frmPromise.style.display='none';},false);frmPromise.onsubmit=function(event){window.event.stopPropagation();window.event.preventDefault()};NSB.addProperties(frmPromise);function LoadPromise(){savethefunction_rvar="";NSB.WaitCursor(True);promiseL.Top=promiseT.Height;promiseL.Height=frmPromise.Height-promiseT.Height;promiseL.deleteItem("all");promiseL.refresh();var sPromise;sPromise=GetSetting("promise");$("#promiseT h1").text(sPromise);var sScriptures;sScriptures=GetSetting(sPromise);var sLines;sLines=Split(sScriptures,VM);var i,sVerse;for(i=0;i<=sLines.length-1;i++){sVerse=sLines[i];sVerse=Replace(sVerse,"\\"," \\ ");promiseL.addItem(sVerse);}
promiseL.refresh();promiseL_ref.scrollTo(0,0,0);NSB.WaitCursor(False);}
promiseL.onclick=function(i){savethefunction_rvar="";if(TypeName(i)=="object"){return savethefunction_rvar;}
var strPromise,strBook,sChapters,sRecord;var strBible,strChapter,strStart,strEnd,strVerses;var strBookID,strVerses1,sKey;strPromise=promiseL.getItem(i);strBible=GetSetting("bible");strPromise=strBible+" \\ "+strPromise;SaveSetting("reading",strPromise);strBible=MvField(strPromise,1,"\\");strBook=MvField(strPromise,2,"\\");strVerses=MvField(strPromise,3,"\\");strBible=Trim(strBible);strBook=Trim(strBook);strVerses=Trim(strVerses);strChapter=MvField(strVerses,1,":");strVerses1=MvField(strVerses,2,":");strChapter=Trim(strChapter);strVerses1=Trim(strVerses1);strStart=MvField(strVerses1,1,"-");strEnd=MvField(strVerses1,2,"-");strStart=Trim(strStart);strEnd=Trim(strEnd);sKey="biblebook_"+strBook;sRecord=GetSetting(sKey);strBookID=MvField(sRecord,1,"*");sChapters=MvField(sRecord,2,"*");SaveSetting("bookid",strBookID);SaveSetting("chapters",sChapters);SaveSetting("book",strBook);SaveSetting("chapter",strChapter);SaveSetting("startverse",strStart);SaveSetting("endverse",strEnd);SaveSetting("bookid",strBookID);sourceForm="frmPromise";ChangeForm(frmVerses);return savethefunction_rvar;}
frmPromise.onshow=function(){savethefunction_rvar="";LoadPromise();return savethefunction_rvar;}
promiseT.onclick=function(choice){savethefunction_rvar="";if(TypeName(choice)=="object"){return savethefunction_rvar;}
switch(True){case((choice)=="home"):ChangeForm(frmPromises);break;case((choice)=="refresh"):LoadPromise();}
return savethefunction_rvar;}
