...
function Main(){NSB.WaitCursor(True);NSB.ShowProgress("Working on it...");NSB.MsgBoxStyle="";sqlList=[];dbName="bibleshow.db";dbObj=SqlOpenDatabase(dbName);if(dbObj==0){NSB.MsgBox("Error opening bibleshow database!");}
NSB.WaitCursor(False);NSB.ShowProgress(False);}
var dbName;var dbObj;var catFrom,sqlList,rCnt,query,recid,isnew;var DB_REAL="REAL";var DB_INTEGER="INTEGER";var DB_BLOB="BLOB";var DB_TEXT="TEXT";var DB_FLOAT="FLOAT";var DB_NUMERIC="NUMERIC";var VM=Chr(253);var FM=Chr(254);var Quote=Chr(34);var DQuote=Chr(34)+Chr(34);var InsertCommands,UpdateCommands,dbExport,dbJSON,dbImport;function NormalizeText(sText){savethefunction_rvar="";sText="<span style="+Quote+"font-weight:normal;font-size:small;"+Quote+">"+sText+"</span>";return sText;return savethefunction_rvar;}
...
function ListViewAddTitleDescriptionIcon(lstView,Title,Description,Icon){savethefunction_rvar="";var sText;sText="<h2>"+Title+"</h2><p>"+Description+"</p>";lstView.addItem(sText);return savethefunction_rvar;}
...
function ListViewFixIcons(lstView){savethefunction_rvar="";lstView.innerHTML=Replace(lstView.innerHTML,"ui-li-has-thumb","ui-li-icon");return savethefunction_rvar;}
function ListViewHasIcon(lstView){savethefunction_rvar="";lstView.innerHTML=Replace(lstView.innerHTML,"ui-li-has-thumb","ui-li-has-icon");return savethefunction_rvar;}
...
switch(True){case((choice)=="Back"):case((choice)=="arrow-l"):ChangeForm(frmDatabase);break;case((choice)=="Restore"):case((choice)=="check"):dbJSON=txtRestore.value;dbJSON=Trim(dbJSON);if(Len(dbJSON)==0){NSB.MsgBox("Please replace the text with the copied text from the 'Backup' E-Mail!");return;}
...
window.addEventListener('load',function(){frmSettings.style.display='block';NSB.List_jqm_init14('lstWhatToDo1','',true,'100%',false);lstWhatToDo1_ref=new IScroll(lstWhatToDo1_scroller,{mouseWheel:true,scrollbars:true,bounce:true,zoom:false});lstWhatToDo1.refresh=function(){if(typeof lstWhatToDo1_ref!='undefined')setTimeout(NSB.refresh,100,lstWhatToDo1_ref)};NSB.addProperties(lstWhatToDo1,lstWhatToDo1_scroller);(function(){var d,i,s=''.split(',');for(i=0;i<s.length;i++){if(trim(s[i])!=''){d=document.getElementById('lstWhatToDo1_'+i);(function(i){d.onclick=function(){ChangeForm(document.getElementById(s[i]))}})(i)}}})()
...
var strItem;strItem=lstWhatToDo1.getItem(i);switch(True){case((i)==0):ChangeForm(CategoryList);break;case((i)==1):ChangeForm(MyInstitutions,"slide");break;case((i)==2):ChangeForm(MyAccounts,"slide");break;case((i)==3):ChangeForm(frmProfile);}
return savethefunction_rvar;}
frmSettings.onshow=function(){savethefunction_rvar="";lstWhatToDo1.Top=hBar1.Height;ListViewClear(lstWhatToDo1);ListViewAddTitleDescription(lstWhatToDo1,"Categories","Maintain categories to group transactions.");ListViewAddTitleDescription(lstWhatToDo1,"Institutions","Maintain financial institutions.");ListViewAddTitleDescription(lstWhatToDo1,"Accounts","Maintain accounts.");ListViewAddTitleDescription(lstWhatToDo1,"Profile","Update user profile.");lstWhatToDo1.Height=frmSettings.Height-hBar1.Height;lstWhatToDo1.refresh();return savethefunction_rvar;}
hBar1.onclick=function(choice){savethefunction_rvar="";if(TypeName(choice)=="object"){return savethefunction_rvar;}
switch(True){case((choice)=="Home"):case((choice)=="home"):ChangeForm(WhatToDo);}
return savethefunction_rvar;}
window.addEventListener('load',function(){frmDatabase.style.display='block';NSB.List_jqm_init14('lstWhatToDo2','',true,'100%',false);lstWhatToDo2_ref=new IScroll(lstWhatToDo2_scroller,{mouseWheel:true,scrollbars:true,bounce:true,zoom:false});lstWhatToDo2.refresh=function(){if(typeof lstWhatToDo2_ref!='undefined')setTimeout(NSB.refresh,100,lstWhatToDo2_ref)};NSB.addProperties(lstWhatToDo2,lstWhatToDo2_scroller);(function(){var d,i,s=''.split(',');for(i=0;i<s.length;i++){if(trim(s[i])!=''){d=document.getElementById('lstWhatToDo2_'+i);(function(i){d.onclick=function(){ChangeForm(document.getElementById(s[i]))}})(i)}}})()
...
var strItem;strItem=lstWhatToDo2.getItem(i);switch(True){case((i)==0):NSB.MsgBox(ContinueBackUp,"This will backup your database and email it to your email address. Please ensure you are connected to the internet.",4+32,"Confirm Backup");break;case((i)==1):ChangeForm(frmRestore);break;case((i)==2):NSB.MsgBox(ResetData,"This will clear all the payments data in database, are you sure?",4+32,"Confirm Reset");}
return savethefunction_rvar;}
frmDatabase.onshow=function(){savethefunction_rvar="";lstWhatToDo2.Top=hBar2.Height;ListViewClear(lstWhatToDo2);ListViewAddTitleDescription(lstWhatToDo2,"Backup","Backup the database and email it as JSON object.");ListViewAddTitleDescription(lstWhatToDo2,"Restore","Restore database from JSON object.");ListViewAddTitleDescription(lstWhatToDo2,"Reset","Clear all the database payments and analysis.");lstWhatToDo2.Height=frmDatabase.Height-hBar2.Height;lstWhatToDo2.refresh();return savethefunction_rvar;}
...
sEmailAddress=Replace(sEmailAddress,'\n',"");sEmailAddress=Replace(sEmailAddress,'\r',"");sEmailAddress=Replace(sEmailAddress,'\n',"");if(Len(sEmailAddress)==0){NSB.ShowProgress(False);NSB.MsgBox("Your email address has not been configured, please update your profile settings first!");return;}
dbJSON=JSON.stringify(dbExport);NSB.ShowProgress(False);localStorage.setItem("backup",dbJSON);email(sEmailAddress,"MyMoney.Show BackUp "+dateadd("s",0,new Date()),dbJSON);return savethefunction_rvar;}
window.addEventListener('load',function(){frmProfile.style.display='block';NSB.MultiInput_init('profileM');NSB.addProperties(profileM);NSB.addProperties(profileT);NSB.addDisableProperty(profileT);if(typeof(profileT.onclick)=='function'){if(typeof(profileT_left)!='undefined')profileT_left.onclick=function(){profileT.onclick(profileT_left.getAttribute('nsbvalue'))};if(typeof(profileT_right)!='undefined')profileT_right.onclick=function(){profileT.onclick(profileT_right.getAttribute('nsbvalue'))}};frmProfile.style.display='none';},false);frmProfile.onsubmit=function(event){window.event.stopPropagation();window.event.preventDefault()};NSB.addProperties(frmProfile);var sfirstname,slastname,semail,smobileno;profileT.onclick=function(choice){savethefunction_rvar="";if(TypeName(choice)=="object"){return savethefunction_rvar;}
switch(True){case((choice)=="Save"):case((choice)=="check"):sfirstname=profileM.getValue(1);slastname=profileM.getValue(2);semail=profileM.getValue(3);smobileno=profileM.getValue(4);sfirstname=Trim(sfirstname);slastname=Trim(slastname);semail=Trim(semail);if(Len(sfirstname)==0){NSB.MsgBox("Please specify a first name!");return;}
if(Len(slastname)==0){NSB.MsgBox("Please specify a last name!");return;}
if(Len(semail)==0){NSB.MsgBox("Please specify an email address!");return;}
localStorage.setItem("email",semail);sfirstname=InSingleQuote(sfirstname);slastname=InSingleQuote(slastname);semail=InSingleQuote(semail);smobileno=InSingleQuote(smobileno);var dbRecord;dbRecord=new Object();dbRecord["ID"]="1";dbRecord["FirstName"]=sfirstname;dbRecord["LastName"]=slastname;dbRecord["Email"]=semail;dbRecord["MobileNo"]=smobileno;var dbUpdate;dbUpdate=new Object();dbUpdate["ID"]="1";UpdateRecord("Users",dbRecord,dbUpdate);break;case((choice)=="Back"):case((choice)=="arrow-l"):ChangeForm(frmSettings);}
return savethefunction_rvar;}
function CheckProfileError(transaction,results){savethefunction_rvar="";NSB.ShowProgress(False);NSB.MsgBox("SQL Error: "+results.code+": "+results.message);return savethefunction_rvar;}
frmProfile.onshow=function(){savethefunction_rvar="";profileM.Left=0;profileM.Top=42;profileM.Width=frmProfile.Width-20;NSB.ShowProgress("Working on it...");profileM.setValue(1,"");profileM.setValue(2,"");var sqlList;sqlList=[];query="SELECT * FROM Users WHERE ID = 1;";sqlList[0]=[query,CheckProfile,CheckProfileError];Sql(dbObj,sqlList);return savethefunction_rvar;}
function CheckProfile(transaction,results){savethefunction_rvar="";var row,i,tusers;tusers=results.rows.length-1;if(_jsCint(tusers)==-1){profileM.setValue(1,"");profileM.setValue(2,"");profileM.setValue(3,"");profileM.setValue(4,"");}else if(_jsCint(tusers)==0){row=results.rows.item(0);sfirstname=row["FirstName"];slastname=row["LastName"];semail=row["Email"];smobileno=row["MobileNo"];profileM.setValue(1,sfirstname);profileM.setValue(2,slastname);profileM.setValue(3,semail);profileM.setValue(4,smobileno);localStorage.setItem("email",semail);}
NSB.ShowProgress(False);return savethefunction_rvar;}
...
function Bible_CreateTable(sTableName){var tblStructure;tblStructure=new Object();tblStructure["VerseID"]="INTEGER";tblStructure["Book"]="INTEGER";tblStructure["Chapter"]="INTEGER";tblStructure["Verse"]="INTEGER";tblStructure["Details"]="TEXT";CreateTable(sTableName,tblStructure,"VerseID","VerseID");CreateTableIndexes(sTableName,"Verse,Chapter,Book");}
