...
function Main(){SaveSetting("onbooks","0");SaveSetting("onpromise","0");SaveSetting("bible","King James");NSB.MsgBoxStyle="";sqlList=[];dbName="bibleshow.db";dbObj=SqlOpenDatabase(dbName);if(dbObj==0){NSB.MsgBox("Error opening bibleshow database!");}}
...
NSB.WaitCursor(True);dbImport=JSON.parse(dbJSON);SQLImport(dbImport,dbObj,restoreComplete);}
return savethefunction_rvar;}
function restoreComplete(){savethefunction_rvar="";NSB.WaitCursor(False);ChangeForm(WhatToDo);return savethefunction_rvar;}
frmRestore.onshow=function(){savethefunction_rvar="";txtRestore.Left=10;txtRestore.Top=42;txtRestore.Height=frmRestore.Height-restoreT.Height-10;txtRestore.Width=frmRestore.Width-20;txtRestore.value=GetSetting("backup");return savethefunction_rvar;}
...
var strItem;strItem=lstWhatToDo2.getItem(i);switch(True){case((i)==0):NSB.MsgBox(ContinueBackUp,"This will backup your database and email it to your email address. Please ensure you are connected to the internet.",4+32,"Confirm Backup");break;case((i)==1):ChangeForm(frmRestore);}
return savethefunction_rvar;}
frmDatabase.onshow=function(){savethefunction_rvar="";lstWhatToDo2.Top=hBar2.Height;ListViewClear(lstWhatToDo2);ListViewAddTitleDescription(lstWhatToDo2,"Backup","Backup the database and email it as JSON object.");ListViewAddTitleDescription(lstWhatToDo2,"Restore","Restore database from JSON object.");lstWhatToDo2.Height=frmDatabase.Height-hBar2.Height;lstWhatToDo2.refresh();return savethefunction_rvar;}
...
NSB.WaitCursor(True);dbExport=SQLExport(dbObj,dbName,backupComplete);}
function backupComplete(){savethefunction_rvar="";var sEmailAddress;sEmailAddress=GetSetting("email");if(sEmailAddress==null){sEmailAddress="";}
sEmailAddress=Replace(sEmailAddress,'\n',"");sEmailAddress=Replace(sEmailAddress,'\r',"");sEmailAddress=Replace(sEmailAddress,'\n',"");if(Len(sEmailAddress)==0){NSB.WaitCursor(False);NSB.MsgBox("Your email address has not been configured, please update your profile settings first!");return;}
dbJSON=JSON.stringify(dbExport);SaveSetting("backup",dbJSON);NSB.WaitCursor(False);email(sEmailAddress,"Bible.Show BackUp "+dateadd("s",0,new Date()),dbJSON);return savethefunction_rvar;}
window.addEventListener('load',function(){frmProfile.style.display='block';NSB.MultiInput_init('profileM');NSB.addProperties(profileM);NSB.addProperties(profileT);NSB.addDisableProperty(profileT);if(typeof(profileT.onclick)=='function'){if(typeof(profileT_left)!='undefined')profileT_left.onclick=function(){profileT.onclick(profileT_left.getAttribute('nsbvalue'))};if(typeof(profileT_right)!='undefined')profileT_right.onclick=function(){profileT.onclick(profileT_right.getAttribute('nsbvalue'))}};frmProfile.style.display='none';},false);frmProfile.onsubmit=function(event){window.event.stopPropagation();window.event.preventDefault()};NSB.addProperties(frmProfile);var sfirstname,slastname,semail;profileT.onclick=function(choice){savethefunction_rvar="";if(TypeName(choice)=="object"){return savethefunction_rvar;}
switch(True){case((choice)=="Save"):case((choice)=="check"):sfirstname=profileM.getValue(1);slastname=profileM.getValue(2);semail=profileM.getValue(3);sfirstname=Trim(sfirstname);slastname=Trim(slastname);semail=Trim(semail);if(Len(sfirstname)==0){NSB.MsgBox("Please specify a first name!");return;}
...
SaveSetting("email",semail);sfirstname=InSingleQuote(sfirstname);slastname=InSingleQuote(slastname);semail=InSingleQuote(semail);var dbRecord;dbRecord=new Object();dbRecord["ID"]="1";dbRecord["FirstName"]=sfirstname;dbRecord["LastName"]=slastname;dbRecord["Email"]=semail;var dbUpdate;dbUpdate=new Object();dbUpdate["ID"]="1";UpdateRecord("Users",dbRecord,dbUpdate);break;case((choice)=="home"):ChangeForm(WhatToDo);}
return savethefunction_rvar;}
function CheckProfileError(transaction,results){savethefunction_rvar="";NSB.MsgBox("SQL Error: "+results.code+": "+results.message);return savethefunction_rvar;}
frmProfile.onshow=function(){savethefunction_rvar="";profileM.Left=0;profileM.Top=42;profileM.Width=frmProfile.Width-20;profileM.setValue(1,"");profileM.setValue(2,"");profileM.setValue(3,"");var sqlList;sqlList=[];query="SELECT * FROM Users WHERE ID = 1;";sqlList[0]=[query,CheckProfile,CheckProfileError];Sql(dbObj,sqlList);return savethefunction_rvar;}
function CheckProfile(transaction,results){savethefunction_rvar="";var row,i,tusers;tusers=results.rows.length-1;if(_jsCint(tusers)==-1){profileM.setValue(1,"");profileM.setValue(2,"");profileM.setValue(3,"");}else if(_jsCint(tusers)==0){row=results.rows.item(0);sfirstname=row["FirstName"];slastname=row["LastName"];semail=row["Email"];profileM.setValue(1,sfirstname);profileM.setValue(2,slastname);profileM.setValue(3,semail);SaveSetting("email",semail);}
return savethefunction_rvar;}
